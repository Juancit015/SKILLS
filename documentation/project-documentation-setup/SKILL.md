...
- **Verificación de arranque (smoke test obligatorio):** ejecutar CADA servicio ejecutable que se documente — entry point principal (`.py`), scripts (`serve.sh`, seeds, `manage.py`), servicios auxiliares — hasta la frontera de credenciales o red (o con valores de prueba), verificando que arrancan en el runtime documentado. Si un servicio falla por bug o incompatibilidad del proyecto: **reportarlo SIEMPRE al usuario con el mensaje de error, la causa raíz y el fix sugerido (la decisión de arreglar es del mantenedor — la skill NO toca código)**. Si no se puede ejecutar (sin entorno), declararlo explícitamente en el resumen.
  - **Auditoría de la URL de red (obligatoria en todo smoke test):** si el proceso emite peticiones HTTP, listar en el análisis el método + la URL COMPLETA + el status de cada una. Ante un fallo de red, clasificarlo ANTES de concluir: `401/403/400` de la API real = fallo ESPERADO con credenciales falsas (fin normal del smoke test, no es defecto); en cambio `404`, `405`, `502`, redirects o URL visiblemente mal formada = **anómalo**: sospechar del DEFAULT de una variable de entorno (URL base mal formada, puerto/path incorrecto) que rompe también con credenciales VÁLIDAS, no culpar al usuario o al token.
  - **Causa raíz con segunda prueba:** ante un fallo anómalo, no conformarse con la primera explicación plausible: validar la hipótesis de forma independiente (petición directa con `curl`/`head` y credenciales ficticias al endpoint sospechoso, inspección del código de la librería instalada en el venv del smoke test, comparación contra el endpoint oficial). Documentar en el resumen las pruebas que descartan/confirman.
  - **Fix sugerido con formato verificado:** si el default sospechoso apunta a un gateway/proxy/servicio de terceros, verificar el formato de ruta REAL de ese proveedor ANTES de escribir la solución (p. ej. un gateway puede enrutar `<token>/<método>` y NO `bot<token>/<método>`: el fix correcto es la URL sin el sufijo de la librería) y ofrecer también la URL oficial estándar de la librería si existe. Si el formato no es verificable, declararlo como incertidumbre en lugar de inventar el fix.
  - **Clasificación del arranque en el resumen:** distinguir SIEMPRE "arranque verificado hasta la frontera de red" (fail-fast y arranque del proceso OK, red no comprobada por credenciales falsas) de "arranque completo verificado": declarar cuál se alcanzó, y si el fallo observado era esperado (credenciales falsas) o anómalo (defecto del proyecto). El fracaso en la frontera de red NO constituye verificación plena del arranque.

Si el programa adopta (o hereda) un runtime que rompe librerías (ej. Python 3.14 + python-telegram-bot 21.x → `RuntimeError: There is no current event loop`), la doc:
